...
// 插入版身banner DOM (圖+連結)
 function insertbodyBN(blockNumFirst1, blockNumLast1, targetDom){
 	...
 	function inHtml() {
 		newArr.forEach(function (item) {
 			html += `
 			<div class="col-4  hoversacle p-1 p-xl-2"><a href="`+ item.Link.Url +`" target="_blank"><img
 								 src="`+ item.Img.Src +`"></a></div>`
 		});
 	}
     document.querySelector(targetDom).innerHTML = html;
 }
...
window.onload = function() {
	...
	// banner
	insertbodyBN(438,447, '#pd_bn');
	// 商品
	insertPD(30,39, '#pd_brand');
	insertPD(30,39, '#pd_01');
	insertPD(30,39, '#pd_02');
    insertPD(30,39, '#pd_03');
	insertPD(30,39, '#pd_04');
	insertPD(30,39, '#pd_05');
    insertPD(30,39, '#pd_06');
	insertPD(30,39, '#pd_07');
};