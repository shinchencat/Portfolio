	...
	function inHtml() {
		newArr.forEach(function (item) {
			...
			(item.Link.Text3 == "") ? 
				...
				<h6>`+ item.Link.Text2 +`</h6>
				<h5>`+ item.Link.Text +`</h5>
				<p class="price"><s>`+ LinkText3 +`</s><span>$<strong>`+item.Link.Text1 +`</strong></span></p>
						</a>
					</li> `
...
// 插入版身banner DOM (圖+連結)
 function insertbodyBN(blockNumFirst1, blockNumLast1, targetDom){
 	...
 	function inHtml() {
 		newArr.forEach(function (item) {
 			html += `
 			<div class="col-6 col-lg-3 hoversacle p-1 px-lg-2"><a href="`+ item.Link.Url +`" target="_blank"><img src="`+ item.Img.Src +`"></a></div>`
 		});
 	}
     document.querySelector(targetDom).innerHTML = html;
 }
	...
	function inHtml() {
		newArr.forEach(function (item, index) {
			html += `<li class="swiper-slide">
					...
					<h6>`+ item.Link.Text2 +`</h6>
					<div class="box-img"><img src="`+ item.Img.Src +`"></div>
						<h5>`+ item.Link.Text +`</h5>
						<p class="price">$<strong>`+item.Link.Text1 +`</strong></p>
					</a>
				</li>`
		});
	}
...
function insertBrandSlider(blockNumFirst1, blockNumLast1, targetDom){
    let html = '';
    let newArr = [];
	dataNode(newArr, blockNumFirst1, blockNumLast1);
	(newArr.length > 0) && (inHtml());
...
window.onload = function() {
	...
	// 強打輪播
	insertMainSlider(42,51, '#mainSlider ul');
	// 第二個輪播
	//insertMainSlider(33,42, '#swiper-new2 ul');
	// 品牌輪播
	insertBrandSlider(64,73, '#sb_01');
	insertBrandSlider(220,229, '#sb_02');
	insertBrandSlider(231,240, '#sb_03');
	insertBrandSlider(242,251, '#sb_04');
	// banner
	insertbodyBN(31,40, '#pd_bn');
	// 商品
	insertPD(53,62, '#pd_kol');
	insertPD(88,97, '#pd_02');
	insertPD(99,108, '#pd_01');
};